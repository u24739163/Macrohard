 ...
 * Works with VioletAbyss's API endpoints
 */
...
async function loadProductDetails() {
    try {
        ...
        const response = await fetch('Requested.php', {
            method: 'POST',
            headers: {
...
function displayPrices(prices) {
    ...
    document.getElementById('best-retailer').textContent = `at ${prices[0].retailer_name}`;
...
function createRetailerCard(priceData, isBestDeal) {
    ...
    card.innerHTML = `
        ...
        <div class="retailer-header">
            <img src="${priceData.logo_url || 'placeholder-logo.png'}" alt="${priceData.retailer_name}" class="retailer-logo">
        </div>
        <div class="retailer-price">
            <span class="price">R${priceData.price.toFixed(2)}</span>
            <span class="shipping">${priceData.shipping_cost ? `R${priceData.shipping_cost} Shipping` : 'Free Shipping'}</span>
        </div>
        <div class="retailer-availability ${priceData.availability === 'In Stock' ? 'in-stock' : 'out-of-stock'}">
            <i class="fas fa-${priceData.availability === 'In Stock' ? 'check' : 'times'}-circle"></i>
            ${priceData.availability}
        </div>
        <div class="retailer-actions">
            <a href="${priceData.product_url}" class="btn primary-btn retailer-btn" target="_blank">View Deal</a>
            <button class="btn-icon wishlist-retailer" data-retailer-id="${priceData.retailer_id}" title="Add to Wishlist">
...
function displayReviews(reviews) {
    ...
    for (let i = 5; i >= 1; i--) {
        ...
        const percentText = document.querySelector(`[data-rating="${i}"] .rating-percentage`);
